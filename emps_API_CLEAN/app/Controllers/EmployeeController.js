...
const employeeUnpaidWithID = (ctx) => {
    return new Promise((resolve, reject) => {
        const query = `
                       SELECT unpaid_hours
                        FROM 
...
const updateTotalHours = (ctx) => {
    return new Promise((resolve, reject) => {
        const query = `
                        UPDATE employee
                        SET unpaid_hours = unpaid_hours + ?
                        WHERE employeeID = ?`;
        dbConnection.query({
            sql: query,
            values: [ctx.params.newHours, ctx.params.employeeID]
        }, (error, tuples) => {
            if (error) {
